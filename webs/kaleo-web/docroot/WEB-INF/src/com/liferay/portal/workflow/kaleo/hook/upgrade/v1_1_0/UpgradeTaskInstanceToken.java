...
public class UpgradeTaskInstanceToken extends UpgradeProcess {
	...
	protected void doUpgrade() throws Exception {
		...
		try {
			...
			ps = con.prepareStatement(
					"select kaleoTaskInstanceTokenId, workflowContext from " +
						"KaleoTaskInstanceToken where workflowContext is not " +
							"null or workflowContext !='' and workflowContext" +
			...
			while (rs.next()) {
				...
				upgradeWorkflowContext(
					newWorkflowContext, kaleoTaskInstanceTokenId);
			}
		}
		finally {
	...
	protected void upgradeWorkflowContext(
			String workflowContext, long kaleoTaskInstanceTokenId)
		throws Exception {
		...
		try {
			...
			ps = con.prepareStatement(
				"update KaleoTaskInstanceToken set workflowContext = ? where " +
					"kaleoTaskInstanceTokenId = ?");

			ps.setString(1, workflowContext);
			ps.setLong(2, kaleoTaskInstanceTokenId);
