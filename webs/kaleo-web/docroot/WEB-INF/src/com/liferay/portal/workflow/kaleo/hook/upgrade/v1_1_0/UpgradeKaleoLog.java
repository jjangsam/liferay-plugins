...
public class UpgradeKaleoLog extends UpgradeProcess {
	...
	protected void doUpgrade() throws Exception {
		...
		try {
			...
			ps = con.prepareStatement(
					"select kaleoLogId, workflowContext from KaleoLog where " +
						"workflowContext is not null or workflowContext !='' " +
			...
			while (rs.next()) {
				...
				upgradeWorkflowContext(newWorkflowContext, kaleoLogId);
			}
		}
		finally {
	...
	protected void upgradeWorkflowContext(
			String workflowContext, long kaleoLogId)
		throws Exception {
		...
		try {
			...
			ps = con.prepareStatement(
				"update kaleoLog set workflowContext = ? where kaleoLogId = ?");

			ps.setString(1, workflowContext);
			ps.setLong(2, kaleoLogId);
